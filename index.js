...
exports.createOpenGraphImage = (createPage, options) => {
  config.init(options)
  const { defaultSize, componentGenerationDir, defaultWaitCondition } =
    config.getConfig()
  const { path, component, context } = options

  const size = { ...defaultSize, ...(options.size || {}) }
  const waitCondition = options.waitCondition
    ? options.waitCondition
    : defaultWaitCondition
  const componentPath = join(
    componentGenerationDir,
    encodeURIComponent(path.split('/').join(''))
  )
  ...
  const ogImageMetaData = {
    path,
    size,
    __ogImageGenerationContext: generationContext
  }
